...
output_dir = '/content/drive/MyDrive/Thesis_Project/Base_Test_Results/Architecture_4'

os.makedirs(output_dir, exist_ok=True)

s1_classes = ['battery', 'glass', 'metal', 'organic_waste', 'paper_cardboard', 'plastic', 'textiles', 'trash']
vlm_classes = ['metal_Grade_A', 'metal_Grade_B', 'plastic_Grade_A', 'plastic_Grade_B'] # Note: fixed casing
...
if not os.path.exists(os.path.join(vlm_grading_dir, 'adapter_config.json')):
    ...
    if checkpoints:
        ...
        raise FileNotFoundError(f"CRITICAL ERROR: No adapter_config.json or checkpoints found in {vlm_grading_dir}.")
...
print("-" * 120)
print(f"{'ITEM':<6} | {'ACTUAL':<17} | {'S1 PREDICT':<15} | {'ACTION':<17} | {'FINAL PREDICT':<17} | {'STATUS'}")
print("-" * 120)
...
for i in range(total_samples):
    ...
    if c1 in ['metal', 'plastic']:
        ...
        with torch.no_grad():
            ...
            # Map index (0-3) to string label using specialized class list
            c2 = vlm_classes[c2_idx]
    ...
    # In ra log Terminal
    print(f"{i+1:03d}    | {true_class:<17} | {c1:<15} | {route_status:<17} | {final_decision:<17} | {final_status}")
...
csv_path = os.path.join(output_dir, 'Arch4_Inference_Routing_Log.csv')
...
print(f"VLM Utilization      : {vlm_usage_count}/{total_samples} samples routed to VLM Grading")
...
report_path = os.path.join(output_dir, 'Architecture_4_Classification_Report.txt')
...
red_colors = ["#ffffff", "#ffebee", "#ef9a9a", "#ef5350", "#c62828"]
...
plt.title('Architecture 4: Precision Grading - Confusion Matrix',
          fontsize=18, fontweight='bold', pad=25, family='serif', color='#c62828')
...
cm_save_path = os.path.join(output_dir, 'Arch4_Confusion_Matrix_Red.png')
...
bars = plt.bar(final_classes, f1_scores, color='#ef5350', edgecolor='#c62828', linewidth=1.5)

plt.title('Architecture 4: F1-Score per Class', fontsize=16, fontweight='bold', family='serif', pad=20)
...
plt.tight_layout()
f1_save_path = os.path.join(output_dir, 'Arch4_F1_Scores_Chart.png')
plt.savefig(f1_save_path, dpi=300, bbox_inches='tight')
plt.close()

# =================================================================================
# EXTRA EVALUATION ARCH 4: PURE WASTE CLASSIFICATION (8 CLASSES - IGNORING GRADES)
# =================================================================================
...
# 2. y_true and y_pred are already Strings in Arch 4, map directly!
...
# 5. Append this finding to the existing Arch 4 text report
...
# 6.1 Confusion Matrix (Teal Theme)
print("\nGenerating 8-Class Confusion Matrix for Arch 4...")
...
plt.title('Architecture 4: Pure Classification (8 Classes)',
...
# Save with Arch4 Prefix
cm_base_save_path = os.path.join(output_dir, 'Arch4_BaseClassification_CM_Teal.png')
plt.savefig(cm_base_save_path, dpi=300, bbox_inches='tight')
plt.close()

# 6.2 F1-Score Bar Chart (8 Classes)
print("Generating 8-Class F1-Score Chart for Arch 4...")
...
plt.title('Architecture 4: Pure Classification F1-Score', fontsize=16, fontweight='bold', family='serif', pad=20)
plt.ylabel('F1-Score', fontsize=12, fontweight='bold', family='serif')
plt.ylim(0, 1.1)
plt.xticks(rotation=45, ha='right', fontsize=11)
...
f1_base_save_path = os.path.join(output_dir, 'Arch4_BaseClassification_F1_Chart.png')
