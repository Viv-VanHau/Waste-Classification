...
# TEST 3: SOFT PENALTY DOMAIN CONSTRAINT (DYNAMIC TRUST MASKING)
# ==============================================================================
...
# Define Soft Penalties for VLM-4 based on CNN base material prediction
# Indices: 0: metal_A, 1: metal_B, 2: plastic_A, 3: plastic_B
# Out-of-domain predictions are penalized (multiplied by 0.35) rather than zeroed out
PENALTY_FACTOR = 0.35
soft_domain_masks = {
    'metal': np.array([1.0, 1.0, PENALTY_FACTOR, PENALTY_FACTOR]),
    'plastic': np.array([PENALTY_FACTOR, PENALTY_FACTOR, 1.0, 1.0])
}
...
print("-" * 130)
print(f"{'INDEX':<7} | {'ACTUAL':<17} | {'S1 PREDICT':<15} | {'ACTION':<22} | {'FINAL PREDICT':<17} | {'STATUS'}")
print("-" * 130)
...
for i in range(total_samples):
    ...
    if c1 in ['metal', 'plastic']:
        ...
        with torch.no_grad():
            ...
            raw_c2_label = vlm_classes[raw_c2_idx]

            # Apply Soft Penalty Mask
            penalty_mask = soft_domain_masks[c1]
            p_vlm_penalized = p_vlm * penalty_mask

            penalized_c2_idx = np.argmax(p_vlm_penalized)
            c2 = vlm_classes[penalized_c2_idx]

            # Tracking sophisticated metrics
            is_cross_domain_raw = ('metal' in c1 and 'plastic' in raw_c2_label) or ('plastic' in c1 and 'metal' in raw_c2_label)
            is_cross_domain_final = ('metal' in c1 and 'plastic' in c2) or ('plastic' in c1 and 'metal' in c2)

            if raw_c2_idx != penalized_c2_idx:
                suppressed_hallucinations += 1
                route_status = "Suppressed Hallucination"
            elif is_cross_domain_final and c2 == true_class:
                soft_rescues += 1
                route_status = "Heroic Soft Rescue"
    ...
    print(f"{i+1:05d}   | {true_class:<17} | {c1:<15} | {route_status:<22} | {final_decision:<17} | {status}")
...
csv_path = os.path.join(output_dir, 'Arch4_Test3_Routing_Log.csv')
...
print(f"Suppressed Hallucinations: {suppressed_hallucinations} out-of-domain false alarms prevented")
print(f"Heroic Soft Rescues     : {soft_rescues} cross-domain errors successfully fixed by VLM")
...
report_path = os.path.join(output_dir, 'Arch4_Test3_Classification_Report.txt')
with open(report_path, 'w') as f:
    f.write("ARCHITECTURE 4: PRECISION GRADING PIPELINE\n")
    f.write("TEST 3: SOFT PENALTY DOMAIN CONSTRAINT\n")
    f.write("="*60 + "\n")
    f.write(f"FPS                     : {fps:.2f} FPS\n")
    f.write(f"Suppressed Hallucinations: {suppressed_hallucinations}\n")
    f.write(f"Heroic Soft Rescues     : {soft_rescues}\n")
    f.write("="*60 + "\n")
    f.write(report_str)

# 8. Visualizations (Teal Theme)
cm = confusion_matrix(y_true, y_pred, labels=final_classes)
teal_colors = ["#ffffff", "#e0f2f1", "#b2dfdb", "#4db6ac", "#00897b", "#004d40"]
...
plt.title('Architecture 4: Soft Penalty Constraint (Confusion Matrix)',
          fontsize=18, fontweight='bold', pad=25, color='#004d40')
...
cm_save_path = os.path.join(output_dir, 'Arch4_Test3_Confusion_Matrix.png')
...
bars = plt.bar(final_classes, f1_scores, color='#4db6ac', edgecolor='#004d40', linewidth=1.5)

plt.title('Architecture 4: F1-Score per Class', fontsize=16, fontweight='bold', pad=20, color='#004d40')
...
plt.tight_layout()
f1_save_path = os.path.join(output_dir, 'Arch4_Test3_F1_Scores.png')
