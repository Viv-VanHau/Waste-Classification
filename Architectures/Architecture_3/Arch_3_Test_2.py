...
# TEST 2: CALIBRATED SOFT FUSION (POST-HOC PRIOR ADJUSTMENT)
# ==============================================================================
...
# Define Calibration Multipliers to counteract class imbalance bias
# Indices: metal_Grade_A is 2, plastic_Grade_A is 6
calibration_weights = np.ones(10)
calibration_weights[2] = 1.25  # 25% boost to metal_Grade_A
calibration_weights[6] = 1.50  # 50% boost to plastic_Grade_A
...
for i in range(total_samples):
    ...
    # Stage 2: Soft Fusion with Post-hoc Calibration
    if cnn_conf < CONFIDENCE_THRESHOLD:
        vlm_calls += 1
        route_action = "Calibrated Fusion"
        ...
        raw_fusion_idx = np.argmax(p_fused)

        # 2. Apply Calibration Multiplier
        p_calibrated = p_fused * calibration_weights

        # Normalize to keep it a valid probability distribution (optional but good practice)
        p_calibrated = p_calibrated / np.sum(p_calibrated)

        final_idx = np.argmax(p_calibrated)
        final_decision = s2_classes[final_idx]

        # Tracking metrics
        if final_idx != raw_fusion_idx:
            calibration_shifts += 1
            route_action = "Shifted by Calib"
...
csv_path = os.path.join(output_dir, 'Arch3_Test2_Routing_Log.csv')
...
report_path = os.path.join(output_dir, 'Arch3_Test2_Classification_Report.txt')
with open(report_path, 'w') as f:
    f.write("ARCHITECTURE 3: CONFIDENCE-ROUTED HYBRID SYSTEM\n")
    f.write("TEST 2: CALIBRATED SOFT FUSION\n")
    f.write("="*60 + "\n")
...
plt.title('Architecture 3: Calibrated Soft Fusion (Confusion Matrix)',
...
cm_save_path = os.path.join(output_dir, 'Arch3_Test2_Confusion_Matrix.png')
...
plt.tight_layout()
f1_save_path = os.path.join(output_dir, 'Arch3_Test2_F1_Scores.png')
